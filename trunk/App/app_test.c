...
#define START_LED		PA_ODR_ODR2
#define SET_LED			PB_ODR_ODR5
#define LED_ON			1
#define LED_OFF			0

void ledOffMode(void)
{
	START_LED = LED_OFF;
	SET_LED = LED_OFF;
}

void ledStandbyMode(void)
{
	START_LED = LED_ON;
	SET_LED = LED_OFF;
}

void ledWorkMode(void)
{
	START_LED = LED_ON;
	SET_LED = LED_ON;
}
	...
	switch(e_mode)
	{
		...
		case MODE_POWER_ON:
			if(u8_powerOnDelay)
			{
				...
				else if(u8_step == 6 && !b_settingSaveFlag)
				{
					u8_powerOnDelay = 30;
					u8_ledDisBuff[0] = 0;	//O
					u8_ledDisBuff[1] = 1;	//1
