...
BOOL b_Start = FALSE;
BOOL b_TimeSet = FALSE;
BOOL b_TempSet = FALSE;
BOOL b_StartOld = FALSE;
BOOL b_TimeSetOld = FALSE;
BOOL b_TempSetOld = FALSE;

UINT8 u8_StartCounter = 0;
UINT8 u8_TimeCounter = 0;
UINT8 u8_TempCounter = 0;
#define COUNTER			3
...
{
	if(b_StartOld != b_Start)
	{
		b_StartOld = b_Start;
		u8_StartCounter = 0;
	}
	else if(u8_StartCounter < COUNTER)
	{
		u8_StartCounter++;
	}
	else
	...
	if(b_TimeSetOld != b_TimeSet)
	{
		b_TimeSetOld = b_TimeSet;
		u8_TimeCounter = 0;
	}
	else if(u8_TimeCounter < COUNTER)
	{
		u8_TimeCounter++;
	}
	else
	{
		b_keyTimeSet = b_TimeSet;
	}
	
	if(b_TempSetOld != b_TempSet)
	{
		b_TempSetOld = b_TempSet;
		u8_TempCounter = 0;
	}
	else if(u8_TempCounter < COUNTER)
	{
		u8_TempCounter++;
	}
	else
	{
		b_keyTempSet = b_TempSet;
	}
}
